...
def getXML(req, media_id):
    ...
    if req == 'RpcApiSubtitle_GetXml':
        ...
        data = {'req': 'RpcApiVideoPlayer_GetStandardConfig', 'media_id': media_id, 'video_format': video_format,
    ...
    try:
        ...
        opener = urllib2.build_opener(cookie)
    opener.addheaders = [
        ('Referer', 'http://static.ak.crunchyroll.com/flash/' + player_revision + '/StandardVideoPlayer.swf'),
...
def vidurl(url):  # experimental, although it does help if you only know the program page.
    res = getHTML(url)
    slist = re.findall(
        '<a href="#" class="season-dropdown content-menu block text-link strong(?: open| ) small-margin-bottom" title="(.+?)"',
        res)
    if slist != []:  # multiple seasons
        if len(re.findall('<a href=".+episode-(01|1)-(.+?)"', res)) > 1:  #dirty hack, I know
            ...
            if url.endswith('/'):
                return url + re.findall('<a href=".+episode-(0' + epnum + '|' + epnum + ')-(.+?)"', res)[
                    slist.index(seasonnum)][1]
            else:
                return url + '/' + re.findall('<a href=".+episode-(0' + epnum + '|' + epnum + ')-(.+?)"', res)[
                    slist.index(seasonnum)][1]
        else:
...
def start(url):
    print 'Booting up...'
    config()
    page_url = "http://www.crunchyroll.com" + url
    ...
    title = unicode((re.findall('<title>(.+?)</title>', html).pop().replace('Crunchyroll - Watch ', '')),
                    encoding='utf-8')
    ...
    host_grr = re.search('fplive\.net',
                         host)  #why host_grr? well, there was a time when fplive videos couldn't be downloaded, so...
    if host_grr:
    ...
    xmllist = unidecode(
        unicode(getXML('RpcApiSubtitle_GetListing', media_id), 'utf-8'))  #could we get rid of unidecode?
    ...
    if '<media_id>None</media_id>' in xmllist:
        ...
        try:
            sub_id = re.findall("id=([0-9]+)' title='.+" + lang.replace('(', '\(').replace(')', '\)') + "'",
                                xmllist).pop()
            hardcoded = False
        except IndexError:
            try:
        ...
        if os.stat(title + '.flv').st_size == 0:
            ...
            if os.stat(title + '.flv').st_size == 0:
                ...
                if os.stat(title + '.flv').st_size == 0:
                    ...
                    sys.exit()
    cmd2 = '.\\video-engine\\FLVExtractCL.exe -v -a -t -o "' + title + ".flv"
    os.system(cmd2)
    cmd3 = 'video-engine\mkvmerge.exe -o' + ' "' + title + '.mkv"' + ' "export' + '" ' + '"' + title + '.264"' + ' --aac-is-sbr 0' + ' "' + title + '.aac"'
    print cmd3
    os.system(cmd3)
    os.remove(title + ".flv")
    os.remove(title + ".264")
